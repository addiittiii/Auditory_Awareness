...
```{r setup}
#| label: descriptive-numeric_stats
#| include: false
...
# Define function to compute summary statistics and save as CSV
...
# Store values for inline use (this is crucial!)
...
Descriptive statistics were computed for both the systole and diastole conditions, including the mean and standard deviations of FFR amplitudes for each. Additionally, the frequencies corresponding to the systole and diastole conditions were also calculated. These statistics provide an overview of the central tendency and variability of the FFR amplitudes and frequencies within each condition. 
```{r}
...
median_systole <- median(frame$systole, na.rm = TRUE)
median_diastole <- median(frame$diastole, na.rm = TRUE)

ggplot(frame_long, aes(x = Condition, y = FFR_Amplitude, fill = Condition)) +
  geom_violin(trim = FALSE, scale = "width", draw_quantiles = c(0.25, 0.5, 0.75)) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) +
  labs(
    x = "Condition", # Simpler x-axis label
    y = "FFR Amplitude (dB)",
    fill = "Condition"
  ) +
  theme_bw() +
  ...
  scale_fill_manual(values = c("Systolic Phase" = "#666666", "Diastolic Phase" = "#111111"))
```
@fig-t-test displays the distribution of FFR amplitude for both conditions. Visually, the median FFR amplitude appears slightly higher in the diastolic phase (M = `r round(median(frame$diastole, na.rm = TRUE), 3)`) compared to the systolic phase (M = `r round(mean(frame$systole, na.rm = TRUE), 3)`). However, both distributions exhibit a similar range and shape, with a somewhat bimodal appearance, suggesting the presence of two peaks in the density of FFR amplitude values within each phase. The interquartile ranges (IQRs), represented by the boxes within the violins, are also comparable between the two conditions, indicating similar variability in the central 50% of the data.
...
systole_low <- quantile(frame$systole, 0.33, na.rm = TRUE)
systole_high <- quantile(frame$systole, 0.67, na.rm = TRUE)

diastole_low <- quantile(frame$diastole, 0.33, na.rm = TRUE)
diastole_high <- quantile(frame$diastole, 0.67, na.rm = TRUE)
...
grid.arrange(systole_plot, diastole_plot, ncol = 2)  # Arrange the two plots in one row with two columns
...
percentile_frame_long <- percentile_frame %>%
  pivot_longer(
    cols = c(systole_category, diastole_category),
    names_to = "condition_type",
    values_to = "category"
  ) %>%
  mutate(
    condition = ifelse(grepl("systole", condition_type), "Systole", "Diastole"),
    ffr_amplitude = ifelse(condition == "systole", systole, diastole)  # Correctly assign FFR amplitude
  )

ggplot(percentile_frame_long, aes(x = condition, y = ffr_amplitude, fill = condition)) +  # x = condition
  geom_bar(stat = "summary", fun = "mean", position = position_dodge(width = 0.8), width = 0.7, color = "black") +
  facet_wrap(~category, scales = "free_y", ncol = 3) +  # Facet by category
  labs(
    x = "Condition (Systole vs. Diastole)",  # Keep overall x-axis title
    y = "FFR Amplitude (dB)",
    fill = "Condition"
  ) +
  theme_bw() +
  theme(
    axis.text.x = element_blank(),        # Remove x-axis text WITHIN facets
    axis.ticks.x = element_blank(),       # Remove x-axis ticks WITHIN facets
    axis.title.x = element_text(size = 10, face = "bold"), # Keep overall x-axis title
  ...
  scale_fill_manual(values = c("Systole" = "#666666", "Diastole" = "#111111")) # Remove breaks
```
